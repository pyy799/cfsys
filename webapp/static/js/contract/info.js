    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/contract/contract_info/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "approval_status_name", "sTitle": "审批状态", "sClass": "center"},
                    ...
                    {"mData":null,"sTitle":"操作", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            ...
                            //var usertype = oData["user_type"];
                            var reapply = $('<a href="edit/'+oData["id"]+'/" class="btn btn-xs btn-danger"> 重新上传</a>');
                            var upload = $('<a href="javascript:click=uploadForm("'+oData["id"]+'");" class="btn btn-xs btn-info"  data-toggle="modal" data-target="#myModal"> 上传盖章版合同</a>');
                            if(usertype==2||usertype==4){
                                if (oData["contract_status_name"]=="审批通过")
                                    element.append(upload);
                                else if(oData["contract_status_name"]=="审批不通过" || oData["approval_status_name"]=="新建合同")
                                    element.append(reapply);
                            }
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/contract/contract_info/data/");
            grid.submitFilter();
        });
