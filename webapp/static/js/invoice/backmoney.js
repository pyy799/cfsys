var TableBackmoney = function () {
    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "contract_money", "sTitle": "合同金额"},
                    {"mData": "back_total", "sTitle": "回款总额"},
                    {"mData": "invoice_no_back_total", "sTitle": "发票未回款"},
                    {"mData": "contract_no_back_total", "sTitle": "合同未回款"},
                    {"mData": null, "sTitle": "详情", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var detail = $('<a href="/invoice/backmoney_of_contract/' + oData["contract"] +"/"+ '">' + "详情" + '</a>');
                            element.append(detail);
                        }
                    }
                ],
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/invoice/backmoney/data/");
            grid.submitFilter();
        });
