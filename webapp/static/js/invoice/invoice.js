var TableInvoice = function () {
    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "invoice_total", "sTitle": "已开票总额"},
                    {"mData": "no_invoice_total", "sTitle": "未开票总额"},
                    {"mData": null, "sTitle": "详情", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var detail = $('<a href="/invoice/invoices_of_contract/' + oData["contract"] + '/">详情</a>');
                            element.append(detail);
                        }
                    }
                ],
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/invoice/invoice/data/");
            grid.submitFilter();
        });
