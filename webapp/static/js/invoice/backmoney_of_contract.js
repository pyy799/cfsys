var TableInfoBackmoney = function () {
    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/invoice/backmoney_of_contract/data/"+$("#contract").val() + "/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "money", "sTitle": "回款金额"},
                    {"mData": "date", "sTitle": "回款日期"},
                    {"mData": null, "sTitle": "详情", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var detail = $('<a href="/invoice/backmoney_detail/' + oData["id"] + "/"+'">详情</a>');
                    ...
                    {"mData": null, "sTitle": "操作", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            ...
                            var modify = $('<a href="javascript:;" class="btn btn-xs btn-info"><i class="fa fa-edit"> 修改</i></a>');
                            var remove = $('<a href="javascript:;" class="btn btn-xs btn-danger"><i class="fa fa-remove"> 删除</i></a>');

                            element.append(modify);    // 将修改按钮添加到界面上
                            element.append(remove);    // 将删除按钮添加到界面上

                            modify.on('click', function () {
                                var id = oData["id"];
                                location.href = "/invoice/backmoney_modify/" + id+"/";
                            });
                            remove.on('click', function () {
                                var id = oData["id"];
                                var con = confirm("确定删除吗?");
                                if (con) {
                                    $.get("/invoice/backmoney/delete/" + id+"/" , function (data) {

                                        if (data.success) {
                                            $.growlService("删除成功", {type: "success"});
                                            location.href = "/invoice/backmoney_of_contract/";
                                        } else {
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/invoice/backmoney_of_contract/data/" + $("#contract").val() + "/");
            grid.submitFilter();
        });
