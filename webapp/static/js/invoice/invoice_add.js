var TableExample = function () {
    ...
    var handleRecords = function () {
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "null", "sTitle": "操作", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            ...
                            var add = $('<a href="javascript:;" class="btn btn-sm btn-success" data-toggle="modal" data-target="#myModal"><i class="fa fa-check">添加</i></a>');
                            //警告
                            element.append(add);    // 将添加按钮添加到界面上
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/invoice/invoice_add/data/");
            grid.submitFilter();
        });
