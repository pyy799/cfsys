    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/invoice/invoice_application/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "apply_date", "sTitle": "申请日期"},
                    {"mData": "status_name", "sTitle": "状态"},
                       ...
                       {"mData": "null", "sTitle": "操作", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            ...
                            modify.on('click', function () {
                                location.href = "/invoice/invoice_application/modify/" + oData['id'] + "/";
                            });
                            remove.on('click', function () {
                                var id = oData["id"];
                                var con = confirm("确定删除吗?");
                                if (con) {
                                    $.get("/invoice/invoice_application/delete/" + id + "/", function (data) {
                                        if (data.success) {
                                            $.growlService("删除成功", {type: "success"});
                                            location.href = "/invoice/invoice_application";
                                        } else {
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/invoice/invoice_application/data/");
            grid.submitFilter();
        });
