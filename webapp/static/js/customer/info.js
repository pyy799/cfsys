    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/customer/customer_info/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": "name", "sTitle": "客户名称"},
                    {"mData": "customer_id", "sTitle": "财务账号"},
                    {"mData": "business", "sTitle": "企业类型"},
                    {"mData": "salesman", "sTitle": "销售"},
                    {"mData": "customer_service", "sTitle": "客服"},
                    // {"mData": "contract_number", "sTitle": "关联合同", "sClass": "center",
                    //     "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    //         var element = $(nTd).empty();
                    //         for(var i = 0;i < oData["contract_number"].length;i++){
                    //             var detail = $('<a href="/contract/contract_detail/'+oData["contract_number"][i][0]+'">'+oData["contract_number"][i][1]+'</a>');
                        ...
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            ...
                            if(usertype != 2){
                                ...
                                modify.on('click', function () {
                                    var id = oData["id"];
                                    location.href = "/customer/customer_info/modify/" + id + "/";
                                });
                                remove.on('click', function () {
                                    var id = oData["id"];
                                    var con = confirm("确定删除吗?");
                                    if (con) {
                                        $.get("/customer/customer_info/delete/" + id + "/", function (data) {
                                            if (data.success) {
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/customer/customer_info/data/");
            grid.submitFilter();
        });
