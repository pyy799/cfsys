    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/finance/finance_modify/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": null, "sTitle": "数据修改", "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                        var element = $(nTd).empty();
                        var modify = $('<a href="javascript:;" class="btn btn-xs btn-info">修改</a>');
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reload
            grid.setUrl("/finance/finance_modify/data/");
            grid.submitFilter();
        });
