    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/finance/finance_settle/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": null, "sTitle": "结账", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var settle = $('<a href="javascript:;" class="btn btn-xs btn-info">结账</a>');
                            element.append(settle);

                            settle.on('click', function () {
                                //var id = oData["id"];
                                //location.href = "/finance/settle/" + id + "/";
                                $.get("/finance/settle/" + oData["id"] + "/", function (data) {
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/finance/finance_settle/data/");
            grid.submitFilter();
        });
        ...
        $("#btnSettleAll").on('click', function () {
            ...
            $.get("/finance/settle/" + mids + "/", function (data) {
                if (data.success) {
                    $.growlService("生成账单成功！", {type: "success"});
                    grid.getDataTable().ajax.reload();
                }
                else {
                    $.growlService("生成账单失败！", {type: "danger"});
                }
            });
        })
    };
