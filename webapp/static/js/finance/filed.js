    ...
    var handleRecords = function () {
        ...
        grid.init({
            ...
            dataTable: {
                ...
                ajax: {
                    url: "/finance/finance_filed/data/"
                },
                ...
                "aoColumns": [ //这个属性下的设置会应用到所有列，按顺序没有是空,bVisible是否可见
                    ...
                    {"mData": null, "sTitle": "合同日期", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var timerange = [oData["start_date"], oData["end_date"]].join(" - ");
                            element.append(timerange);
                        }
                    },
                    {"mData": "total_money", "sTitle": "合同总价"},
                    {"mData": null, "sTitle": "归档", "sClass": "center",
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            var element = $(nTd).empty();
                            var filed = $('<a href="javascript:;" class="btn btn-xs btn-info">归档</a>');
                            element.append(filed);

                            filed.on('click', function () {
                                var con = confirm("确定归档？");
                                if (con) {
                                    $.get("/finance/filed/" + oData["id"] + "/", function (data) {
                                        if (data.success) {
                                            $.growlService("归档成功", {type: "success"});
                                            grid.getDataTable().ajax.reload();
                                        } else {
                                            $.growlService("归档失败", {type: "danger"})
                                        }
                                    })
                                }
                            });
        ...
        tableWrapper.on('click', '.filter-submit', function (e) {
            e.preventDefault();
            //only set no reaload
            grid.setUrl("/finance/finance_filed/data/");
            grid.submitFilter();
        });
        ...
        $("#btnFiledAll").on('click', function () {
            ...
            $.get("/finance/filed/" + cids + "/", function (data) {
                if (data.success) {
                    $.growlService("合同归档成功！", {type: "success"});
                    grid.getDataTable().ajax.reload();
                }
                else {
                    $.growlService("合同归档失败！", {type: "danger"});
                }
            });
        })
    };
